...
def parse():
    ...
    parser.add_argument(
        "--shapes", type=int, default=0, help="num different key+lock types"
    )
    parser.add_argument("--keys", type=int, default=0, help="number keys vector")
    parser.add_argument("--locks", type=int, default=0, help="number locks vector")
    parser.add_argument(
        "--prob-goal",
        type=float,
        default=1.0,
...
def main():
    ...
    assert args.shapes <= args.keys or args.shapes <= args.locks, "There cannot be more shapes than keys or locks"
    ...
    instance_name = f"grid-{args.x}rows-{args.y}cols-{args.shapes}shapes-{args.keys}keys-{args.locks}locks"
    ...
    if args.typed:
        ...
        shape_facts = [f"(objshape {shape})" for shape in shapes]
    ...
    for k in range(args.keys):
        if random.random() < args.prob_goal:
            ...
            p = random.choice([p for p in positions])
            goal_facts.append(f"(at_robot {pos_name(p)})")
