...
def add_noise_to_distilled(distilled: Dict, noise: float, logger) -> Dict:
    noisy = dict()
    for key in set(distilled.keys()) - set(['fval']):
        noisy.update({key: deepcopy(distilled[key])})

    noisy.update(fval=dict(), unknown=dict())
    assert 'fval' in distilled
    for inst in distilled['fval']:
                ...
                if unverified_nodes:
                    ...
                    if not verify_only:
                        if not (solve_path / fname.name).exists():
                            noisy_distilled = add_noise_to_distilled(distilled, noise, logger)
                            add_new_instance(inst, fname, solve_path, noisy_distilled, logger)
...
def _parse_arguments():
    parser = argparse.ArgumentParser(description='Incremental learning of grounded PDDL models (Noise Model)')
    ...
    default_seed = 0
    default_noise = 0.05
    unknown = parser.add_argument_group('random generation of unknown atoms')
